...
const CreateMeetingModal: React.FC<CreateMeetingModalProps> = ({
    ...
    useEffect(() => {
        ...
        if (lockedCustomer) {
            ...
            setCampus(lockedCustomer.campus || 'Hanoi');
        } else {
    ...
    }, [customers, selectedCustomerKey, lockedCustomer]);
    ...
    const hasConflict = useMemo(() => {
        ...
        return hasTeacherConflict(teacherId, meetingDateTime);
    }, [teacherId, meetingDateTime]);

    const handleSelectCustomer = (customer: MeetingCustomerOption) => {
        setSelectedCustomerKey(customer.key);
        setQuery(customer.name);
        setCampus(customer.campus || 'Hanoi');
    };
    ...
    const handleCreate = () => {
        ...
        if (!campus.trim()) {
            setError('Vui lòng nhập/chọn cơ sở.');
            return;
        }
        if (!teacherId) {
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-2xl bg-white rounded-xl shadow-2xl border border-slate-200">
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
                    <h3 className="text-lg font-bold text-slate-900">Tạo lịch hẹn mới</h3>
                    ...
                    <div>
                        ...
                        {lockedCustomer ? (
                            <div className="px-3 py-2 rounded-lg border border-slate-200 bg-slate-50 text-sm font-medium text-slate-700">
                                {lockedCustomer.name} - {lockedCustomer.phone}
                            </div>
                        ) : (
                ...
                <div className="px-6 py-4 border-t border-slate-200 flex justify-end gap-2">
                    ...
                    <button onClick={handleCreate} className="px-4 py-2 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg">
                        Lưu lịch hẹn
                    </button>
                </div>
            </div>
